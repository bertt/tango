...
public class Accessibility_EditorFunctions
{
	...
	static public void UpdateCheckVersion()
	{
		if (wwwVersionCheck.isDone)
		{
			...
			if (wwwVersionCheck.text != null && wwwVersionCheck.text.Length > 0 && wwwVersionCheck.text.StartsWith("VERSION"))
			{
				int index = wwwVersionCheck.text.IndexOf("\n", 0);
				string latestVersionString = wwwVersionCheck.text.Substring(8, index - 8);
				...
				if (float.TryParse(latestVersionString, out latestVersion))
				{
					...
					if (!automaticCheck || ignoreVersion.CompareTo(latestVersionRaw) != 0)
					{
						...
						if (automaticCheck)
							UAP_UpdateWindow.Init();
						FoundLatestVersion(latestVersion, latestVersionRaw, wwwVersionCheck.text);
					}
				}
			}
			else
	...
	static public void DrawAccessibilityIcon(int instanceID, Rect selectionRect)
	{
		...
		// Check here
		UAP_BaseElement[] go = GameObject.FindObjectsOfType(typeof(UAP_BaseElement)) as UAP_BaseElement[];

		foreach (UAP_BaseElement g in go)
		{
			if (g.gameObject.GetInstanceID() == instanceID)
			{
