...
public class AccessibleUIGroupRoot : MonoBehaviour
{
	...
	public class Accessible_UIElement
	{
		...
		public void CalculatePositionOrder(UAP_BaseElement uiElement, int backupIndex)
		{
			...
			{
				AccessibleUIGroupRoot groupRoot = obj.GetComponentInParent<AccessibleUIGroupRoot>();
...
*/
			...
			{
				Debug.LogWarning("[Accessibility] Could not find any UI transforms on UI element " + obj.name + " that the accessibility plugin can understand - ordering UI elements by order of initialization.");
	...
	public void RefreshContainer()
	{
		...
		UAP_BaseElement[] elements = gameObject.GetComponentsInChildren<UAP_BaseElement>();
	...
	void Register_Item(UAP_BaseElement item)
	{
		...
		for (int i = 0; i < count; ++i)
		{
			...
			else if (order == m_AllElements[i].m_PositionOrder)
			{
				...
				if (secondaryOrder < m_AllElements[i].m_SecondaryOrder)
				{
					m_AllElements.Insert(i, newElement);
					added = true;
					break;
	...
	public void SelectItem(UAP_BaseElement element, bool forceRepeatItem = false)
	{
		for (int i = 0; i < m_AllElements.Count; ++i)
		{
			...
			if (accessElement.m_Object == element)
			{
				...
				UAP_AccessibilityManager.MakeActiveContainer(this, forceRepeatItem);
				return;
			}
		}
	}
	public string GetContainerName(bool useGameObjectNameIfNone = false)
	{
		if (m_ContainerName.Length > 0)
		{
			...
			if (IsNameLocalizationKey())
				return Localization.Get(m_ContainerName);
#endif
