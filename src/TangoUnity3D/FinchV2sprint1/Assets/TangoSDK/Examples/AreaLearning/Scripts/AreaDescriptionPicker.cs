...
public class AreaDescriptionPicker : MonoBehaviour, ITangoLifecycle
{
    ...
    {
        // for now hardcode the guid to download
        var guid = "735dd2f9-c8fc-2ac0-9cc8-4404fbd5ac58";

        Debug.Log("ajax get adf " + guid + " from azure....");

        StartCoroutine(Download(guid));
    }
    ...
    public void PostToAzure()
    {
        Debug.Log("ajax post to azure...." + m_curAreaDescriptionUUID);

        // first make a copy of the file
        var areaDescription = AreaDescription.ForUUID(m_curAreaDescriptionUUID);
        var result = areaDescription.ExportToFile("/sdcard/");

        Debug.Log("ajax exportToFile result:" + result);

        var bytes = File.ReadAllBytes("/sdcard/" + m_curAreaDescriptionUUID);
    ...
    {
        Debug.Log("ajax number of bytes: " + bytes.Length);
        var form = new WWWForm();
        form.AddBinaryData("files", bytes, m_curAreaDescriptionUUID + ".bin");
        var server= "https://finchserver.azurewebsites.net/api/Areas";
        using (var www = UnityWebRequest.Post(server, form))
        {
            yield return www.Send();
            ...
            {
                Debug.Log("ajax Form upload complete! bytes: " + www.uploadedBytes);
            }
        }
    }
